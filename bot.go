...
func (d *BasicBot) init() {
	...
	executeFlow := func(m *tb.Message) {
		...
		input := &TextInput{UserId: m.Sender.ID, Username:m.Sender.Recipient(), ChatId: strconv.Itoa(int(m.Chat.ID)), Text: m.Text}
		if m.Location != nil {
...
func (d *BasicBot) RenderStartFrame(m *tb.Message) {
	...
	for k := range d.abilities {
		...
		d.TelegramBot.Handle(&inlineBtn, func(c *tb.Callback) {
			d.UfManager.ExecuteFlow(d, strconv.Itoa(c.Sender.ID), c.Sender, &TextInput{UserId: c.Sender.ID, Username:c.Sender.Recipient(), ChatId: strconv.Itoa(int(m.Chat.ID)), Text: k, Location: Location{Lat: c.Message.Location.Lat, Lng: c.Message.Location.Lng}})
		})
	}
